function benchmark(max, steps, algorithm, arrayBuilder) {
  const increasePerStep = Math.floor(max / steps);
  let currentAmount = 1;
  console.log(`About to test ${algorithm.name}`);
  console.log(`Testing input size up to ${max}, with ${steps} steps`);
  while (currentAmount < max) {
    let arrayToTest = generateArray(arrayBuilder, currentAmount);
    ...
    currentAmount += increasePerStep;
  }

  function generateArray(creator, length) {
    let n = 0;
    let array = [];
    while (array.length < length) {
      array.push(creator(array.length, length));
  }
    return array;
  }
}

// benchmark(5000000, 20, sort, (index, max) => Math.floor(Math.random() * max));
// benchmark(5000000, 20, randomAccess, (index, max) => Math.floor(Math.random() * max));
...
function randomAccess(array) {
  let indexToAccess = Math.floor(Math.random() * array.length);
  let accessedItem = array[indexToAccess];
}
...
function arrayInsert(array) {
  ...
  array.splice(middleIndex, 0, 'New Item');
}
...
function findDuplicatesGood(array) {
  ...
  let values = {};
  array.forEach(i => {
    if (values[i]) duplicates.push(i);
    else values[i] = true;
  });
}

function binarySearch(array) {
  elementToFind = Math.floor(Math.random() * array.length);
  let index = binaryIndexOf.call(array, elementToFind);
...
function travellingSalesman(array) {
  const solution = solver({
    body: {
      points: array.map(coords => ({x: coords[0], y: coords[1]})),
      animate: false,
      algorithm: 'Iterative Two Opt',
    }
  });

  var solver = function (req) {
      // Source: https://github.com/dzhang55/travelling-salesman/blob/master/solver.js
      ...
      switch (req.body.algorithm) {
          ...
          case 'Hill Climber':
              if (!req.body.iterations) {
                  console.log({err: 'Please fill out all fields.'});
                  return;
              }
